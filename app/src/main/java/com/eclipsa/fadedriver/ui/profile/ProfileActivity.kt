package com.eclipsa.fadedriver.ui.profile
...
import com.eclipsa.fadedriver.databinding.ActivityProfileBinding
import com.eclipsa.fadedriver.extension.loadingImage
...
import com.eclipsa.fadedriver.utils.*
import com.eclipsa.fadedriver.utils.Utils.handleApiError
import com.eclipsa.fadedriver.utils.Utils.hide
import com.eclipsa.fadedriver.utils.Utils.snackbar
import com.eclipsa.fadedriver.utils.Utils.toast
import com.eclipsa.fadedriver.utils.Utils.visible
...
//                    addProfileOrEditProfile()
    ...
    private fun savePreferences(data: ResultNew) {
        ...
        Prefrences.savePreferencesString(this@ProfileActivity, Constants.USER_ID, data.id.toString())
        Prefrences.savePreferencesString(this@ProfileActivity, Constants.ROLE_ID, data.roleId!!.toString())
        Prefrences.savePreferencesString(this@ProfileActivity, Constants.USER_MOBILE_NO, data.mobile!!.toString())
        ...
        Prefrences.savePreferencesString(this@ProfileActivity, Constants.ADDRESS_LINE_1, data.addressLine1!!)
        Prefrences.savePreferencesString(this@ProfileActivity, Constants.POSTAL_CODE, data.postalCode!!)
        Prefrences.savePreferencesString(this@ProfileActivity, Constants.ADDRESS_LINE_2, data.addressLine2!!)
        ...
        Prefrences.savePreferencesString(this@ProfileActivity, Constants.CITY_ID, data.city!!.id.toString())
        Prefrences.savePreferencesString(
            this@ProfileActivity,
            Constants.CITY_NAME,
            data.city.name!!
    ...
    private fun setUserData() {
        ...
        val city = Prefrences.getPreferences(this, Constants.CITY_NAME)
...
//            binding.barberProofPhoto.isEnabled = it
        ...
        loadingImage(this, Constants.STORAGE_URL + barberIdPhotoPath!!,
            binding.ivBarberIdPhoto, false)
    ...
    private fun addProfileOrEditProfile() {
        ...
        if (binding.tvEditProfile.text.equals("Update Profile")) {
...
//                params["state"] = selectedStateID
            ...
            if (name.isEmpty()) {
                ...
                if (barberProofPhotoName.isEmpty()) {
                    ...
                    if (mCurrentPhotoPath != null) {
                        val imagePath: String =
                            Utils.compressImage(this@ProfileActivity, mCurrentPhotoPath!!)
                                .toString()
                        body = prepareFilePart("profile", imagePath)
                        parts.add(body!!)
                    }
                    ...
                    if (barberIdPhotoPath != null) {
                        val imagePath: String = Utils.compressImage(this@ProfileActivity, barberIdPhotoPath!!).toString()
...
//                    params["state"] = selectedStateID
    ...
    private fun showSingleImage(uri: Uri, imageFor: String) {
        ...
        when (imageFor) {
            driverImage -> {
                mCurrentPhotoPath = com.eclipsa.fadedriver.utils.FileUtil.getPath(context, uri)
                Glide.with(context).load(mCurrentPhotoPath).into(binding.ivUserProfile)
            }

            driverIdPhoto -> {
                barberIdPhotoPath = com.eclipsa.fadedriver.utils.FileUtil.getPath(context, uri)
                cropImage(uri)
                //Glide.with(context).load(barberIdPhotoPath).into(binding.ivBarberIdPhoto)
            }
            driverProofPhoto -> {
                barberProofPhotoPath = com.eclipsa.fadedriver.utils.FileUtil.getPath(context, uri)
                cropImage(uri)
                //Glide.with(context).load(barberProofPhotoPath).into(binding.ivBarberProofPhoto)
            }
        }
    ...
    private fun cropImage(uriTemp: Uri) {
        ...
        if (currentSelectedImage == driverImage) {
            ...
            CropImage.activity(uriTemp)
                .setGuidelines(CropImageView.Guidelines.ON).setAspectRatio(2, 1).start(this)
        }
